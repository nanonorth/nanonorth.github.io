...
import SecantMethod from "./Method/Secant_Method";
import SecantChart from "./Chart/Secant_Chart";
...
function Secant() {
  ...
  const handleCalculate = () => {
    try {
      ...
      const method = new SecantMethod(equation, x0Val, tol);
    ...
    >
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          ...
          >
            <InputBox label="X0 (จุดแรก)" value={x0} setValue={setX0} step={0.5} />
          ...
          >
            <strong>สูตร:</strong> x₍ᵢ₊₁₎ = xᵢ - f(xᵢ) × (xᵢ - xᵢ₋₁) / (f(xᵢ) - f(xᵢ₋₁))
          </div>
    ...
    <div style={{ overflowX: "auto" }}>
      ...
      >
        <thead>
          ...
          >
            {["Iteration", "x", "f(x)", "Error"].map((head) => (
              <th
                key={head}
                style={{
                  padding: "14px 10px",
