...
function BisectionGraph({ equation, iterations, XL, XR }) {
  ...
  const CustomDot = ({ cx, cy, fill, onClick, label }) => {
    return (
        ...
        >
          ...
          {latestXM && (
            <ReferenceLine
              ...
              stroke="transparent"
              label={{
                value: `XM = ${latestXM >= 1000 ? latestXM.toExponential(4) : latestXM.toFixed(6)}`,
          ...
          {latestXM && xmY !== null && (
            <ReferenceDot
              ...
              stroke="#1E1F27"
              strokeWidth={3}
              onClick={() => setSelectedPoint({ label: 'XM (Root)', x: latestXM, y: xmY, color: '#FFD700' })}
              style={{ cursor: 'pointer' }}
              shape={<CustomDot fill="#FFD700" onClick={() => setSelectedPoint({ label: 'XM (Root)', x: latestXM, y: xmY, color: '#FFD700' })} />}
            />
          )}
        </LineChart>
      </ResponsiveContainer>
